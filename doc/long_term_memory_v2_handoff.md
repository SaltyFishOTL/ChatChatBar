...
- 主聊天请求把非空Archive作为独立`system`消息放在历史之后，不再与RAG、世界书、HEAD合并。发送前重新读取失败会直接报错；若内存预算表明有Archive正文但编译结果为空，或最终消息列表缺少Archive标记，请求会在联网前被阻止。调试控制台直接显示最终Request JSON中Archive/HEAD各自是否已发送。
...
- 聊天历史组装先判断正文/有效图片，再追加`[Txx]`。AI生成图片留下的空助手记录、被“排除状态栏”清空的助手记录和其他空助手记录不再产生T前缀占位消息；流式完成但无正文时明确失败，不再持久化空助手回复。
...
- Archive空发送与重复T前缀紧急修复已完成代码、隔离提交JVM/CI和模拟器启动验证；待推送并发行下一版本。真实旧会话最终请求仍待用户安装新版后确认。
...
- 旧聊天历史先拼接`[Txx]`再判断消息是否为空；AI图片记录、纯状态栏和空回复因此变成大量prefix-only助手消息。已改为正文/有效图片先决条件，并禁止空流式回复落库。
...
1. 在存在旧Episode/Arc/Era的真实会话打开完整预览并抓取一次API请求，确认Archive含全部非空正文，历史中没有仅含`[Txx]`的助手消息。
...
- 定向回归通过：全部长期记忆策略、聊天历史空消息策略、Archive独立请求与HEAD-only拒绝、最终JSON标记、PromptAssembler顺序、状态栏排除和时间线提示。
