...
- Multi-character designer output contains character `caption` only; do not request per-character centers. Placement remains automatic by default; manual studio positioning is owned by generation settings and `NovelAiCharacterPositionPolicy` (see chatbar-image-generation-runtime), not AI design text.
...
- V5 quote expansion belongs to `NovelAiV5TextPromptPolicy`, shared by requests and token counting. It follows official web build 3102745/module 46278: ASCII/curly double and single quotes plus Japanese corner quotes, apostrophe boundaries, >30% CJK reversal within each field, first base chunk only, and a trailing `, teXt:` block. Character order and centers stay unchanged by text expansion. Explicit case-insensitive `text:` uses official boundaries and excludes `text::`/spaced colons. Keep stored/designed text unchanged; V4.5 bypasses expansion.
