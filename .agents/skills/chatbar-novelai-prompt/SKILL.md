...
- Studio live completion reuses the same application-scoped `TagSuggestClient` and cache as research. Keep its 250ms UI debounce, cancellation, max-eight candidates, and nonblocking failure state outside prompt text. Bound TagSuggest to three concurrent requests per host and retry HTTP 502/503/504 for three total attempts with staggered delay; persistent gateway failure remains visible and must not be disguised as success.
