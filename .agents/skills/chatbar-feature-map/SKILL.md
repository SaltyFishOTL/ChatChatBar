...
- Message AI format repair: persisted settings/notice in `AppSettings.kt` and `ChatMessage.kt`; prompt in `domain/prompt/PromptTemplates.kt`; exact auxiliary model resolution in `domain/model/EffectiveModelResolver.kt`; streaming request/service and pure replacement rules in `domain/chat/StreamingChatService.kt`, `MessageFormatRepairService.kt`, and `MessageFormatRepairPolicy.kt`; auto/manual orchestration in `ui/chat/ChatViewModel.kt`; long-press, progress, error, and restore UI in `ui/chat/ChatScreen.kt`; global controls in `ui/manage/ManageScreen.kt`.
