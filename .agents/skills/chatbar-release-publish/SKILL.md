...
3. Resolve the immediately previous stable Release tag from remote GitHub Releases, then determine the next unused patch version. Never reuse a tag.
4. Run `.\gradlew.bat test` and `powershell -ExecutionPolicy Bypass -File .\ci.ps1 -SkipAssemble` from `app/`. Fix in-scope failures before publishing.
5. Stage everything with `git add -A`. Review the staged diff, then create one concise Chinese commit covering the update.
6. Build a Release coverage checklist from the complete range `<previous-stable-tag>..HEAD`:
...
7. Write concise release notes entirely from the completed coverage checklist and user perspective:
...
9. Dispatch `.github/workflows/release.yml` with:
   - `versionName`: next unused version without `v`.
   - `releaseNotes`: load the exact Markdown file from step 7.
10. Monitor workflow through completion. On failure, inspect the failed job, fix the cause, commit and push the fix, then dispatch an unused version if a tag was already created.
11. Recheck the final published tag range against the coverage checklist, then run `python .agents/skills/chatbar-release-publish/scripts/verify_release.py --repo SaltyFishOTL/ChatChatBar --version <version>`. Completion requires all gates:
...
12. If verification reports missing, incomplete, or mismatched notes, repair the Release with `gh release edit v<version> --notes-file <file>`, then repeat the range audit and verification. Never report success from workflow inputs, dispatch output, or asset presence alone.
13. Pull the workflow-created version bump with `git pull --ff-only`, remove temporary notes files, then confirm local workspace is clean and matches `origin`.
