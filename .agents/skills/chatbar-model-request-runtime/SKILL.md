...
- For opted-in `http://` model requests, preserve the first system role and serialize non-trailing later system roles as assistant. Merge a trailing strong-prompt system into the preceding user message; if merging is impossible, serialize that tail as user. Never end a cleartext request with an adapted assistant prompt. Do not apply this adaptation to HTTPS or when cleartext access is disabled.
