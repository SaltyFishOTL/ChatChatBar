...
- Studio reads current Anlas and V5 Opus allowance from `/user/subscription` on entry and after successful generation. Keep the V5 count explicitly approximate. Estimate button cost locally with the official pixel-plus-Steps formula and free-sample rules so setting changes stay immediate; Opus exemption requires a single sample, while every image in a batch is charged. Account-fetch failure must not block generation or claim a V5-free request.
